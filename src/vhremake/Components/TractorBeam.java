 ...
 * @author John
 */
public class TractorBeam extends Gun{
    static final long serialVersionUID = 6840357814417805004L;
    ...
    public void obeyMaster(Game game)
    {
        ...
        if (discharge>0)
        {
            ...
            if (hitRa<hitRb)
            {
                if (hit!=null&&hit.MASTER!=MASTER&&hit.MASTER!=null)
                {
                    hit.MASTER.xspeed+=(x-hit.x)/10000;
                    hit.MASTER.yspeed+=(y-hit.y)/10000;
                    
                    if (hit.disTo(x, y)<100)
                    {
                        hit.MASTER.xspeed/=1.2;
                        hit.MASTER.yspeed/=1.2;
                        hit.MASTER.xspeed+=(MASTER.xspeed-hit.MASTER.xspeed)/8;
                        hit.MASTER.yspeed+=(MASTER.yspeed-hit.MASTER.yspeed)/8;
                    }
                }
            }
            else
            {
                if (aHit!=null)
                {
                    ...
                    if (aHit.disTo(x, y)<100)
                    {
                        aHit.xspeed/=1.2;
                        aHit.yspeed/=1.2;
                        aHit.xspeed+=(MASTER.xspeed-aHit.xspeed)/8;
                        aHit.yspeed+=(MASTER.yspeed-aHit.yspeed)/8;
                    }
                }
            }
    ...
    public void render(GL2 g, double vX, double vY)
    {
        ...
        if (discharge>0)
        {
            g.glBegin( GL2.GL_LINES);
                g.glColor4f(0f,1f,0f,1f);
                g.glVertex2f(0,0);
                g.glVertex2d(-hitR-50+r.nextInt(100),-50+r.nextInt(100));
                g.glVertex2f(0,0);
                g.glVertex2d(-hitR-50+r.nextInt(100),-50+r.nextInt(100));
                g.glVertex2f(0,0);
                g.glVertex2d(-hitR-50+r.nextInt(100),-50+r.nextInt(100));
