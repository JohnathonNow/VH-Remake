 ...
 */
extends Node2D
class_name RectangularHull
    static var serialVersionUID = -7228373935924493628L
    func RectangularHull(int x, int y, int width, int height):
    ...
    public final func constructPoints():
        for (var i = shape.xpoints[0]+8 i <= shape.xpoints[1]-8; i+=16)
        {
        ...
        for (var i = shape.ypoints[0]+8 i <= shape.ypoints[2]-8; i+=16)
        {
    ...
    var cloned = new RectangularHull((int)x,(int)y,shape.xpoints[1]*2,shape.ypoints[2]*2)
