 ...
 */
extends Node2D
class_name TriangularHull
    static var serialVersionUID = 981289526117837655L
    func TriangularHull(int x, int y, int size):
        this.x = x;
        this.y = y;
        shape.addPoint(0,-size/2);
        shape.addPoint(-(int) (size*Math.sqrt(3)/2f),0);
        shape.addPoint(0,size/2);
//        for (var i = -width/2+8 i <= width/2-8; i+=16)
//        {
...
//        }
    ...
    var cloned = new TriangularHull((int)x,(int)y,shape.ypoints[2]*2)
