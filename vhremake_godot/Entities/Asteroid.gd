...
class_name Asteroid
    var shape
    var radius = 5
    func Asteroid(double x, double y, double vx, double vy, double radius):
        this.x = x;
        this.y = y;
        xspeed = vx;
        yspeed = vy;
        ...
        this.radius = radius;
        shape = new Polygon();
        var numSides = random.nextInt(5)+5
        var degChange = Math.PI*2/numSides
        var deg = func for(var i = 0 i <numSides; i++):
            var size = radius*(50+random.nextInt(50))/90
